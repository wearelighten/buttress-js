    ...
    return new Promise((resolve, reject) => {
      ...
      if (!validation.isValid) {
        if (validation.missing.length > 0) {
          ...
          reject({statusCode: 400, message: `CALL: Missing field: ${validation.missing[0]}`});
          return;
        }
        if (validation.invalid.length > 0) {
          this.log(`ERROR: Invalid value: ${validation.invalid[0]}`, Route.LogLevel.ERR);
          reject({statusCode: 400, message: `CALL: Invalid value: ${validation.invalid[0]}`});
          return;
        }

        this.log(`ERROR: CALL: Unhandled Error`, Route.LogLevel.ERR);
        reject({statusCode: 400, message: `CALL: Unhandled error.`});
        return;
      }
